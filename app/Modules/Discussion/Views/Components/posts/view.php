    ...
    <div class="card-footer bg-transparent">
        ...
        <div class="float-end">
            ...
            <?php if (isset($_SESSION['user']) && $_SESSION['user']['id'] == $post['opponent_id']): ?>
                <a href="/<?= $language ?>/<?= $post['opponent_username'] ?>/discussion/<?= $post['discussion_id'] ?>/edit" class="btn btn-outline-warning btn-sm ms-3" title="<?= __('edit') ?>"><i class="bi bi-pencil-fill"></i></a>
                <form method="POST" action="/<?= $language ?>/<?= $post['opponent_username'] ?>/discussion/<?= $post['discussion_id'] ?>/delete" class="d-inline-block">
